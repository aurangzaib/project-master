...
// default ctor
BottleDetection :: BottleDetection () { imagePath = "/Meeting-7/original-2.bmp";}
// ctor
BottleDetection :: BottleDetection (const string imagePath): imagePath(imagePath) {}
// find the hough line
void BottleDetection :: findLinePoints () {
    ...
    for (int loopVar = 0; loopVar < lines.size(); loopVar++) {
        ...
        line(outputImage, pt1, pt2, Scalar(0, 0, 255), 1, CV_AA);               // draw line using point1 and point2. red color
    }
...
void BottleDetection :: findLineUniquePoints () {
    ...
    if (linePoints.size() < 3) {
        ...
        for (int loopVar = 0; loopVar < linePoints.size() - 1; loopVar++) {
            cout << endl << "comparing " << linePoints.at(loopVar) << " with " << linePoints.at(loopVar + 1);
            if ((linePoints.at(loopVar) - linePoints.at(loopVar + 1)) > valueRange / 2)
