...
void bottleCapDetection(void) {
	...
	for (int loopVar = 0; loopVar < arraySize; loopVar++) {
		*(img + loopVar) = imread(masterproject::prjdir + "/Meeting-5/cap-teach-2.bmp", CV_LOAD_IMAGE_COLOR);
	}
	...
	for (int loopVar = 0; loopVar < arraySize; loopVar++) {
		...
		adaptiveThreshold(thresh, threshAdapt, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 5, 3);
		adaptiveThreshold(thresh, threshAdapt2, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 3, 1);
		adaptiveThreshold(thresh, threshAdapt3, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 7, 1);
		...
		for (size_t i = 0; i < bottleCaps.size(); i++) { // just two caps at a time
			...
			// putText(*(img + loopVar), to_string(loopVar), (int) bottleCaps[i][0], (int) bottleCaps[i][1]);
			
			cout << "x: " << bottleCaps[i][0] << " y: " << bottleCaps[i][1] << " r: " << bottleCaps[i][2] << endl;
			// draw the circle for bottle cap
			if (abs(bottleCaps[i][0]) < 1000 && abs(bottleCaps[i][1]) < 1000 && abs(bottleCaps[i][1]) > 50 && abs(bottleCaps[i][2]) < 40) {
				circle(
					*(img + loopVar),					// image
					Point(bottleCaps[i][0], bottleCaps[i][1]),					// x, y of circle (to be drawn)
				...
				circle(*(img + loopVar), Point(bottleCaps[i][0], bottleCaps[i][1]), 1, Scalar(0, 0, 255), 3, 8, 0);
			}
		}
		...
		for (size_t i = 0; i < loopSize; i++) {
			...
			if (abs(bottleCaps[i][0] - bottleCaps[i + 2][0]) < 550) { // x difference
				if (abs(bottleCaps[i][1] - bottleCaps[i + 2][1]) < 150) { // y difference
					if (abs(bottleCaps[i][2] - bottleCaps[i + 2][2]) < 10) { // radius difference 
		...
		for (size_t i = 0; i < bottleRadius.size(); i++) {
			...
			// draw the center of bottle cap
			circle(*(img + loopVar), Point(c[0], c[1]), 1, Scalar(255, 255, 255), 3, 8, 0);
		}
		*/
	}
